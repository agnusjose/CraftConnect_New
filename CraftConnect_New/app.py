...
def manage_orders():
    ...
    try:
        with sqlite3.connect("craftconnect.db", timeout=10) as conn:
           ...
           orders.phone, orders.email, orders.payment_status,orders.order_date
                FROM orders
                JOIN products ON orders.product_id = products.id
...
def track_order():
    ...
    SELECT orders.id, products.name AS product_name, orders.quantity, 
           ...
           orders.phone, orders.email, orders.status, products.image,orders.size
    FROM orders
    JOIN products ON orders.product_id = products.id
    WHERE orders.user_id = ?
''', (user_id,))
...
def place_order():
    ...
    SELECT products.id, products.name, products.price, cart.quantity, 
           products.manufacturer_id, products.stock_limit, products.image, cart.size
    FROM cart 
    JOIN products ON cart.product_id = products.id
    WHERE cart.user_id=?
""", (user_id,))
    ...
    INSERT INTO orders (user_id, product_id, quantity, total_price, 
                        payment_status,order_date,address, phone, email, status, product_image, size) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)
""", (user_id, product_id, quantity, total_price, payment_status,indian_time,
      address, phone, email, "Processing", product_image, item[7]))  # item[7] now contains size
