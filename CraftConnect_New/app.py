...
def place_order():
    ...
    # Fetch products from the cart along with manufacturer_id and stock_limit
    cursor.execute("""
        SELECT products.id, products.name, products.price, cart.quantity, products.manufacturer_id, products.stock_limit
        FROM cart 
        JOIN products ON cart.product_id = products.id
        WHERE cart.user_id=?
    """, (user_id,))
    ...
    for item in cart_items:
        ...
        stock_limit = item[5]  # Fetch stock_limit
